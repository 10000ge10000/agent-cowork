  ...
  handleServerEvent: (event) => {
    ...
    switch (event.type) {
      case "session.list": {
        ...
        // 关键修复：不要在自动刷新 session.list 时强制打开/关闭模态框
        // 只有当用户主动操作（pendingStart 状态）时才由 session.status 控制模态框
        // 这样可以避免滚动消息等操作导致模态框被意外关闭
        const hasSessions = event.payload.sessions.length > 0;
        if (!state.pendingStart) {
          // 只有在非启动过程中，才根据会话列表自动显示/隐藏开始模态框
          // 且只在初始加载时（showStartModal 为 false 且没有活跃会话）自动打开
          if (!state.showStartModal && !state.activeSessionId && !hasSessions) {
            set({ showStartModal: true });
          }
        }
