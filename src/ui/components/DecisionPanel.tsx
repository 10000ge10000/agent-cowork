...
export function DecisionPanel({
  ...
  // 使用 key 作为 state 初始化的一部分，避免 effect 中直接 setState
  ...
  if (request.toolName === "AskUserQuestion" && questions.length > 0) {
    return (
      <div className="rounded-2xl border border-accent/20 bg-accent-subtle p-5">
        <div className="text-xs font-semibold text-accent">Claude 的问题</div>
        {questions.map((q, qIndex) => (
          <div key={qIndex} className="mt-4">
            ...
            <div className="mt-3">
              <label className="block text-xs font-medium text-muted">其他</label>
              <input
                type="text"
            ...
            {q.multiSelect && <div className="mt-2 text-xs text-muted">支持多选。</div>}
          </div>
        ))}
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <button
            ...
            onClick={() => onSubmit({ behavior: "deny", message: "用户取消了问题" })}
          >
            取消
          </button>
        </div>
  ...
  return (
    <div className="rounded-2xl border border-accent/20 bg-accent-subtle p-5">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        <button
          className="rounded-full border border-ink-900/10 bg-surface px-5 py-2 text-sm font-medium text-ink-700 hover:bg-surface-tertiary transition-colors"
          onClick={() => onSubmit({ behavior: "deny", message: "用户拒绝授权" })}
        >
          拒绝
        </button>
      </div>
